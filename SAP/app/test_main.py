...
st.sidebar.header("🔄 Connexion SAP BW")

sap = SAPConnector()
if sap.connect():
    st.sidebar.success("Connecté à SAP BW ✅")
    if st.sidebar.button("Charger exemple données SAP"):
        columns, data = sap.read_table("VBAK", ["VBELN", "ERDAT", "ERNAM"], max_rows=10)
        df_sap = pd.DataFrame(data, columns=columns)
        st.subheader("Exemple de données SAP BW")
        st.dataframe(df_sap)
    sap.close()
else:
    st.sidebar.error("Échec connexion SAP ❌")
